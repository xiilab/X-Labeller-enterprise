...
.ztree .level0 span.button[id$=ico] { background: url("images/icon_tree_folder_normal.png") !important; }
		.ztree a.curSelectedNode.level0  span.button[id$=ico] { background: url("images/icon_tree_folder_white.png") !important; }
...
// 	var parent_node;			// jh.sa 210223 parent_node => that.data.parent_node 사용 안함 
	var annotation = {
		pt : $("#annotation"),
		data : {
			initObj : null,
...
// 				var str = $(this).val();
		...
		detail : function(id) {
			...
			if(id == null){
				detail_id = wsObj[that.data.initObj.key].dir.getSelectedNodes()[0].annotation_id;
				label_type = wsObj[that.data.initObj.key].dir.getSelectedNodes()[0].labelType;
		...
		del : function(){
			var that = annotation;
			var nodes = wsObj[that.data.initObj.key].dir.getSelectedNodes();
			var msg = "삭제하시겠습니까?";			
			var deArr = [];
			...
			if(confirm(msg) == true){
				for(var i=0; i< nodes.length; i++){
					deArr.push(that.deleteAnnotation(nodes[i].annotation_id));
				}
		...
		exports : function() {
			...
			let win = window.open(baseUrl + "annotation/exportAnnotation.json?annotation_id="+node.annotation_id);
		...
		deleteAnnotation : function(node_id){
			...
			var ajaxData = {
				annotation_id :  node_id	
			}
			
			if(hide_id == "" || hide_id == null){
				that.data.hide_id += ""+node_id;	
			} else {
				that.data.hide_id += ","+node_id;
			}
		...
		getAnnotationList : function() {
			var that = annotation;
			var parent_node = wsObj[that.data.initObj.key].dir.getNodes()[0];
			var data = {};
			var url = "annotation/getAnnotationList.json";
		...
		setAnnotationResult : function(data, parentNode) {
			...
			$.each(data, function(index, value) {
				var obj = new Object();
				obj.annotation_id = value.annotation_id;
				obj.pId =  "0." + value.annotation_id;
