...
/*       		#videoBBoxList .label_wrap .category_wrap { max-height: 880px; }  */
			#videoBBoxList .label_wrap .category_wrap { max-height: 775px; } 
      	}
      	@media (min-height: 730px) and (max-height:959px){
/*       		#videoBBoxList .label_wrap .category_wrap { max-height: 665px; }  */
			#videoBBoxList .label_wrap .category_wrap { max-height: 560px; } 
      	}      	
      	@media (min-height: 500px) and (max-height:729px){
/*       		#videoBBoxList .label_wrap .category_wrap { max-height: 500px; }  */
			#videoBBoxList .label_wrap .category_wrap { max-height: 395px; } 
      	} 
      	@media (min-height: 1px) and (max-height:499px){
/*       		#videoBBoxList .label_wrap .category_wrap { max-height: 400px; }  */
			#videoBBoxList .label_wrap .category_wrap { max-height: 295px; } 
      	} 	
...
/* 			top: 300px; */
		...
		#videoBBoxList section.tag { width: 486px; height: 100%; position: relative; background-color: #fff; border: 1px solid #e5e5e5; border-bottom: 0; -ms-user-select: none; -moz-user-select: none; -webkit-user-select: none; -khtml-user-select: none; user-select: none; }
		#videoBBoxList section.tag .tag_margin { margin: 0 auto; padding: 10px; }
 		#videoBBoxList section.tag .tag_total { color: #777; font-weight: 400; padding: 20px; }
 		#videoBBoxList section.tag .tag_wrap { width: 100%; height: 90px; margin-top: 1px; border-top: 1px solid #e5e5e5; font-size: 11px; justify-content: center; } 
		#videoBBoxList section.tag .tag_wrap .content { width: 100%; height: auto; max-height: 75px; overflow-y: scroll; overflow-x: hidden; flex-wrap: wrap; -ms-overflow-style: none; }
		#videoBBoxList section.tag .tag_wrap .content::-webkit-scrollbar { display: none; }		
		#videoBBoxList section.tag .tag_wrap .tag { width: 81px; margin: 10px 5px; line-height: 32px; background: #f4f7fc; border: 1px solid #d9d9d9; border-radius: 4px; text-align: center; font-size: 14px; font-weight: 300; color: #555; cursor: pointer; overflow: hidden; white-space: nowrap; text-overflow: ellipsis; }
		#videoBBoxList section.tag .tag_wrap .tag.selected { background-color: #4c84ff; border-color: #2b65e6; color: #fff; box-shadow: 0 3px 4px 0 rgba(159, 189, 255, 0.7); }
		#videoBBoxList .hover_box {border: 1px solid #d9d9d9; text-align: center; z-index: 9999; display: none; position: absolute; border-radius: 2px; box-shadow: 0 2px 3px 0 rgba(0, 0, 0, 0.19); background-color: #ffffff; }
		#videoBBoxList .hover_box div {font-family: Open_Sans; font-size: 13px; color: #555555;}
		#videoBBoxList .hover_div div {word-break: break-word; margin: 2px 5px; white-space: normal; font-weight: 300; font-family: Open_Sans;}		
      </style>
   </head>
   <body id="page-top">
      <!-- Page Wrapper -->
...
// 					var dataSet = "5";
					...
					that.computed.getLabelGroup(that.data.videoObj['datasetId']);
...
// 							var lastPage = Math.floor($("#videoBBoxList .label_title span").html()*1/page_size);
							var lastPage = Math.floor($("#videoBBoxList .label_wrap .label_title").data("cnt")*1/page_size);
							if(page_num>lastPage){
								alert("마지막 페이지 입니다.");
								return false;
							}
							
							var page = $("#videoBBoxList .label_wrap .label_title").data("cnt")*1/page_size;
							if(page == page_num){
								alert("마지막 페이지 입니다.");
								return false;
							} else {
...
// 							that.computed.getLabelDataByDatasetId(page_num, that.data.videoObj['datasetId']);
						...
						$("#videoBBoxList #pagination .last").off("click").on("click",function(){
							var page_size=50;
							var page_num = Math.floor($("#videoBBoxList .label_wrap .label_title").data("cnt")*1/page_size);
// 							that.computed.clearAllData();
// 							that.computed.getLabelDataByDatasetId(page_num, that.data.videoObj['datasetId']);
...
// 		 								that.render.initMetaListFromData(result);
										...
										$("#videoBBoxList .label_wrap .label_title span").html(total_size);
										$("#videoBBoxList .label_wrap .label_title").data("cnt",total_size);
										that.bind.resetView();
						...
						$.ajax({
							...
							success : function(res){
								...
								if(res.result.code == 200){
									...
									$("#videoBBoxList .label_wrap .label_title span").html(tagset.length);
								} else {
									alert(res.result.data)
								}
							},
