...
/*       		border-radius : 10px; */
      	...
      	/* jh.sa 210520 */
		#videoBBox .label_wrap .cate_wp .visibility { width: 20px; height: 20px; display: inline-block; position: relative; top: 0px; background: url('${pageContext.request.contextPath}/resources/images/labeller/ico-eye-on-light.png') center no-repeat; }
		#videoBBox .label_wrap .cate_wp .visibility.off,	
		#videoBBox .label_wrap .cate_wp .visibility:hover { opacity: 1; background: url('${pageContext.request.contextPath}/resources/images/labeller/ico-eye-off-light.png') center no-repeat; cursor: pointer; }
...
/* 	    #videoBBox .label_wrap .category_wrap .cate_wp input { width: 165px; padding: 10px; margin-top: 3px; } */
	    #videoBBox .label_wrap .category_wrap .cate_wp input { width: 390px; padding: 10px; margin-top: 3px; }
      	...
      	/*       	#edit h.lee */
      	#videoBBox .prev_data_wrap { border: 1px solid white; width: 35px; height: 486px; margin-top: 39px; }
		
		/* 		#edit h.lee */
		#videoBBox .next_data_wrap { border: 1px solid white; width: 35px; height: 486px; margin-top: 39px; }  
		
		#videoBBox .prev_data_wrap:hover {
/* 			background: url(images/btn_image_arrow_left_hover.png) left; */
			cursor: pointer; background-color: #eee; opacity: 0.3; 
		}
		
		#videoBBox .next_data_wrap:hover {
					...
					upEvt : () => { // canvas rect mouse up event
						...
						canvas.on('mouse:up', function(e) {
							...
							if(!e.target || e.target != tempRect){
								...
								//edit j.seo
								that.computed.stackHistory(uuid,"added");
								// k.park 201217 active movement
...
//	 							that.computed.initControlBar();	
						    ...
						    // shift + n : 바운딩 박스 생성 
						   	if(evt.shiftKey == true && evt.keyCode == "78"){
								...
								} else if(that.pt.find(".category_wrap .cate_wp").length >= 30){
								...
								var active_obj = videoBbox.data.canvas.getActiveObject();
								if(active_obj != null){
...
//	 							that.computed.getCategoryData(data,uuid);
						 ...
						 // h.lee
						 	// 첫 프레임 이동 단축키
						 	if(evt.ctrlKey == true && evt.shiftKey == true && evt.keyCode == "37"){
								...
								if(labelSelect.length != 0){
									...
									// 라벨클릭 + shift + left arrow
							 		console.log("라벨클릭 + shift + left arrow == 라벨 첫 프레임 이동");
									var uuid = labelSelect.attr("id");
									var startFrm = that.data.meta[uuid]['startFrame'];
									
									// 선택한 라벨 제외 hide 해제 
									if(that.pt.find(".label_wrap .btn_wrap .visibility").hasClass("off") && uuid != that.pt.find(".label_wrap .btn_wrap .visibility")[0].dataset.id){
										that.pt.find(".label_wrap .btn_wrap .visibility").removeClass("off");
										that.computed.visibleMeta(uuid);
									}
									...
									// shift + left arrow
							 		console.log("shift + left arrow == 영상 첫 프레임 이동");
									var frame_input = $(".frame_wrap input[name=curr_frame]");
									var first_frame = "0"
									
									that.data.playerFrame.seekTo({frame: first_frame});	
									that.computed.checkMetaFrame(first_frame);
									that.render.inputCurrFrame(first_frame);
																	
									frame_input.val(first_frame);
								}
						 	}
						 	
						 	// h.lee
						 	// 끝 프레임 이동 단축키
						 	if( evt.ctrlKey == true && evt.shiftKey == true && evt.keyCode == "39"){
								...
								if(labelSelect.length != 0){
									// 라벨클릭 + shift + right arrow
									
							 		console.log("라벨클릭 + shift + right arrow == 라벨 끝 프레임 이동");
									...
									console.log("shift + right arrow == 영상 끝 프레임 이동 ");
							...
							// h.lee
						 	// 라벨 클릭 + shift + b : 이전 클립 삭제  
						 	if( evt.shiftKey == true && evt.keyCode == "66"){
								...
								if(labelSelect.length != 0){
									...
									var canvas = that.data.canvas;
									var uuid = $(".cate_wp.selected")[0].id;
...
// 										canvas.discardActiveObject();
									...
									if(canvas.getActiveObject() == null){
										that.data.canvas.discardActiveObject().renderAll();
										alert("선택된 라벨의 바운딩 박스가 현재 프레임에 없습니다.")
										return;  
									}
									
									var labelNo = $(".cate_wp.selected")[0].id;
									
									var tmp;
									tmp = confirm("삭제하시면 이전 프레임에 대한 바운딩 박스 데이터는 삭제됩니다. 그래도 삭제하시겠습니까?");
									if (tmp == true && canvas.getActiveObject() != null){
										...
										if (frame > meta.startFrame){
											var currArr = meta.endFrame - frame;
											var delArr = frame - meta.startFrame;
											
											meta.startFrame = frame; 
											meta.rectData.splice(0, delArr);
											meta.rectData[0].status = 1;
											
											// undo,redo stack
											that.computed.stackHistory(uuid,"removed");
											
										} else {
											alert("첫 프레임 입니다. 삭제할 이전 프레임이 없습니다.");
											return ;
										}
...
// 										canvas.discardActiveObject();
									...
									var tmp;
									tmp = confirm("삭제하시면 이후 프레임에 대한 바운딩 박스 데이터는 삭제됩니다. 그래도 삭제하시겠습니까?");
									if (tmp == true && canvas.getActiveObject() != null){
										...
										if (frame > meta.startFrame){
											...
											// 첫프레임만 존재하거나 첫 프레임에서 이후 프레임 삭제할 경우
										 	delete that.data.meta[uuid]['startFrmae'];
											delete that.data.meta[uuid]['endFrame'];
											delete that.data.meta[uuid]['rectData'];
...
// 							that.computed.getVideoDataList(page_num,that.data.videoObj['datasetId']);
						...
						$("#videoBBox .prev_data_wrap").off("click").on("click",function(){
							var currDataNum = that.data.videoObj['currDataNum']*1-2;
							var page_size=50;
							if(currDataNum<0){
								alert("처음 데이터입니다.");
						...
						$("#videoBBox .next_data_wrap").off("click").on("click",function(){
							var currDataNum = that.data.videoObj['currDataNum']*1;
							var page_size=50;
							var total_size = $("#videoBBox .position_wrap .total_count span").html()*1;
...
// 								that.bind.keydownEvt();
						...
						// k.park -> THIS CODE BELOW SHOULD BE ADDED KDX
						/* that.pt.find(".category_wrap .cate_wp").off("click").on("click",function(){
							console.log("click!");
							var uuid = $(this).attr("id");
							var color = that.data.colorHash.rgb(uuid);
							// jh.sa 210318 선택한 라벨 제외 hide 해제 
							if(that.pt.find(".label_wrap .btn_wrap .visibility").hasClass("off") && uuid != that.pt.find(".label_wrap .btn_wrap .visibility")[0].dataset.id){
								that.pt.find(".label_wrap .btn_wrap .visibility").removeClass("off");
								that.computed.visibleMeta(uuid);
							}							
				...
				computed : {
					...
					history : function(mode){
						...
						if(mode == "undo"){
					    	...
					    	/* if(that.data.undoStack[that.data.undoStack.length-1].action == "loaded"){
					    		return;
					    	} */
					    	if(that.data.undoStack[that.data.undoStack.length-1].rectData != undefined && that.data.undoStack[that.data.undoStack.length-1].action != "label"){
					    		that.data.redoStack.push(that.data.undoStack.pop());
					    	}
					    	var previous_state = that.data.undoStack[that.data.undoStack.length-1];
					    	if(previous_state == null){
//						    		previous_state = '{}';
					    		console.log("previous_state null : ", that.data.undoStack);	
					    	} else {
					    		if(previous_state.action == "label"){
						    		// label_id, label_name
						    		if(previous_state.label_id != null && previous_state.category != null || previous_state.label_id != null && previous_state.category != "" ){
						    			that.pt.find(".cate_wp#"+previous_state.id).find("input[name='label_name']").val("");
						    			that.data.meta[previous_state.id]["label_id"] = null;
						    			that.data.redoStack.push(that.data.undoStack.pop());
								...
								// 라벨은 있지만 캔버스 객체가 없는 상태일 때
								if(previous_state.rectData == undefined || previous_state.rectData == null){
									var canv = canvas.getObjects();
									var arr = [];
									for(var i=0; i<canv.length; i++){
										// 캔버스에서 객체 지우고 우측 라벨박스 미리보기도 지우기 
										if(canv[i].uuid == previous_state.id){
												arr.push(canv[i]);
												that.data.meta[previous_state.id].rectData = [];
												canvas.remove(canv[i]);
												canvas.renderAll();
												var crop = that.pt.find(".cate_wp#"+canv[i].uuid).find(".preview_canv");
									...
									// 라벨박스만 남아있을 때 라벨박스, 메타 삭제 후 redoStack에 push
									if(arr.length == 0){
										that.pt.find(".cate_wp#"+previous_state.id).remove();
										delete that.data.meta[previous_state.id];
							    		that.data.redoStack.push(that.data.undoStack.pop());
									}
								...
								// 해당 stack의 프레임으로 이동
								that.data.playerFrame.seekTo({frame: previous_state.currFrame});
								that.computed.checkMetaFrame(previous_state.currFrame);
								that.render.inputCurrFrame(previous_state.currFrame);
								
								that.data.meta[previous_state.id]['rectData'] = previous_state.rectData;
								that.data.meta[previous_state.id]['endFrame'] = previous_state.endFrame;
								that.data.meta[previous_state.id]['startFrame'] = previous_state.startFrame;
								
								// 라벨도 있고 캔버스 객체도 있는데 위치를 이동하거나 크기를 변경한 경우
								var canv = canvas.getObjects();
								var currFrame = that.data.playerFrame.get();
								var arrNum = currFrame - that.data.meta[previous_state.id].startFrame;
								for(var i=0; i<canv.length; i++){
									if(canv[i].uuid == previous_state.id){
										canv[i].left = previous_state.rectData[arrNum].left;
										canv[i].top = previous_state.rectData[arrNum].top;
										canv[i].width = previous_state.rectData[arrNum].width;
										canv[i].height = previous_state.rectData[arrNum].height;
										canv[i].selectable = true;
										
										if(that.data.meta[previous_state.id] == null){
											that.computed.initMetaData(previous_state.id);
										}
										that.computed.saveMetaData(canv[i],currFrame,canv[i].uuid);
// 										var newRect = that.computed.rescale(that.data.meta[canv[i].uuid]['rectData'],true);
// 										that.render.renderCropImgIntoLabel(canv[i].uuid,newRect);		
					    	...
					    	console.log("undo => redoStack : ", that.data.undoStack);
...
//						    		console.log("redo => undoStack : ", that.data.undoStack);
					    		...
					    		if(state.action == "add"){
					    			...
					    			if(state.rectData == undefined || state.rectData == null){
										if(that.pt.find(".cate_wp#"+state.id).length == 0){
	 					    				...
	 					    				var color = that.data.colorHash.rgb(uuid);
	 										that.pt.find(".label_wrap .category_wrap [id='"+uuid+"'] .color").css("background-color","rgb("+color+")");
	 										that.pt.find(".label_wrap .category_wrap [id='"+uuid+"'] .category").val(that.data.meta[uuid]['category']);
	 										that.bind.sideEvtCollision();	
	 										that.data.pauseSaving = false;
	 										return;
										}
					    			...
					    			// 해당 stack의 프레임으로 이동
									that.data.playerFrame.seekTo({frame: state.currFrame});
									that.computed.checkMetaFrame(state.currFrame);
									that.render.inputCurrFrame(state.currFrame);
									
									that.data.meta[state.id]['rectData'] = state.rectData;
									that.data.meta[state.id]['endFrame'] = state.endFrame;
									that.data.meta[state.id]['startFrame'] = state.startFrame;
					    			
									var currFrame = that.data.playerFrame.get();
									if(that.data.meta[state.id].startFrame == null || that.data.meta[state.id].startFrame == undefined){
										that.data.meta[state.id]['startFrame'] = state.currFrame;
									}
									var arrNum = currFrame - that.data.meta[state.id].startFrame;
					    			// rectData 있으면 캔버스에 객체 생성
									var temp = {
										uuid : state.id,
										left : state.rectData[arrNum].left,
										top : state.rectData[arrNum].top,
										width : state.rectData[arrNum].width,
										height : state.rectData[arrNum].height
									}					    		
									var rectObj = that.computed.getRectObject(temp);
							    	that.computed.initRect(state.id,rectObj);	
							    	that.computed.saveMetaData(rectObj,currFrame,state.id);
...
// 												that.data.meta[state.id].rectData = [];
									...
									for(var i=0; i<canv.length; i++){
										if(canv[i].uuid == state.id){
											canv[i].left = state.rectData[arrNum].left;
											canv[i].top = state.rectData[arrNum].top;
											canv[i].width = state.rectData[arrNum].width;
											canv[i].height = state.rectData[arrNum].height;
											canv[i].selectable = true;
...
// 											that.render.renderCropImgIntoLabel(canv[i].uuid,newRect);
					...
					// jh.sa 210326
					stackHistory : function(uuid,action){
						const that = videoBBox;
						var meta = that.data.meta;
						var canvas = that.data.canvas;
...
// 					        undo_stack.push(JSON.stringify(canvas));
					        redo_stack = [];
					        var currFrame = String(that.data.playerFrame.get());
					        if(action == "added"){
					        	var data = JSON.parse(JSON.stringify(meta[uuid]));
					        	data.action = "add";
					        	data.currFrame = currFrame;
					        	...
					        	data.currFrame = currFrame;
					        	undo_stack.push(data);
					        	console.log('Object label, state saved', undo_stack);	
					        } else if(action == "loaded"){
					        	var data = JSON.parse(JSON.stringify(meta[uuid]));
					        	data.action = "loaded";
					        	data.currFrame = currFrame;
					        	undo_stack.push(data);
					        	console.log('Object loaded, state saved', undo_stack);						        	
					        }
					...
					copyMetaData : function(copyId){
						...
						console.log("copyMetaData new data : ", meta[uuid]);
...
// 								console.log(res);
					...
					checkMetaFrame : function(frame){
						...
						for(var i=0; i<keys.length; i++){
							...
							//
							//console.log("meta[keys[i]]['visible']::", meta[keys[i]]['visible']);
							//
						...
						for(var i = 0; i < canvasObjects.length; i++){
							...
							//console.log("canvasObjects[i]['visible']::", canvasObjects[i]['visible']);
							// uuid가 일치하면 isExist는 true, 지울 필요 없음
							if(!isExist){
								// delete
						...
						for(var i=0; i<candidateMetaList.length; i++){
							...
							for(var j=0; j<canvasObjects.length; j++){
								// false라면 캔버스에 그려진 객체와 메타 리스트의 객체가 일치하지 않으므로 그려주지 않음
							...
							if(!isExist){
								var arrNum = frame - meta[uuid].startFrame;
								if(meta[uuid]['visible']) {
									that.computed.initMetaRect(uuid,arrNum);
								}
							}
						};
						
						//canvasObjects[j]["visible"]
				...
				// draw something
				render : {
					/* renderLabelNum : function() {
						const that = videoBBox;
						var meta = that.data.meta;
						//console.log("meta::", that.data.meta);
						//console.log("meta.length::", Object.keys(videoBBox.data.meta).length);
						that.pt.find(".label_wrap .label_title span").html(Object.keys(that.data.meta).length);
					}, */
					// jh.sa 210318 라벨박스, 메타 복사
					renderCopyMeta : function(uuid){
						const that = videoBBox;
						var meta = that.data.meta;
						//that.render.renderLabelNum();	
...
//	 									that.root.computed.initEnrichment();
				...
				render : {
					...
					drawAlgorithmList(data){
						...
						for(let i=0, len = data.length; i<len; i++){
							if(data[i].label_type == "VIDEO_BBOX" || data[i].title == "YOLO"){
					...
					drawTaskList(task_list_data, project_data){
						...
						for(let i = 0; i < task_list_data.length; i++){
							...
							for(let j = 0; j < project_data.length; j++){
								if(task_list_data[i].label_type == "VIDEO_BBOX"){
									if(task_list_data[i].project_id === project_data[j].project_id){
										html += "<td class='task_name' data-id='"+task_list_data[i].task_id+"' data-projectId='"+project_data[j].id+"'>"+project_data[j].title+" / "+task_list_data[i].title+"</td>";							
									} else {
										continue;
									}								
								}
