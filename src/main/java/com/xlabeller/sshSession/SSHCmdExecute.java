...
public class SSHCmdExecute{
	private String userName;
	private int port;
	private String host;
	private String pw;
	private Session session;
	private static Logger logger = Logger.getLogger(SSHCmdExecute.class);
	
	public SSHCmdExecute(String userName, int port, String host, String pw) {
...
//					logger.info("cmdExcute Success!" + outputBuffer.toString());
					...
					logger.error("cmdExcute Error!" + errorBuffer.toString());
					result = null;
	...
	// 모든 이미지 검색
		public GpuNodeImageVO imageCheck(String imageName) {
			...
			gpuNodeImageVO.setNode_ip(host);
...
//			unAvailableGpuIndex = unAvailableGpuIndex.substring(0,unAvailableGpuIndex.length());
		...
		public Object callCustomTrain(String projectId, String taskId, String algorithmId, String gpuIndex, String mode, String config) {
			String cmd = 
					"docker run --rm -itd --label gpu_id=" + gpuIndex + " --label "
					+ "xlabeller=t_" + projectId + "_" + taskId + " --name xlabeller_t_" + projectId + "_" + taskId + " -v xlabeller_nfs:/xlabeller ca_" + algorithmId + ":latest"
					+ " python -u run_custom_model.py --action train --mode " + mode + " --params \"" + config + "\" --gpu_id " + gpuIndex 
			...
			if(cmdResult == null) {
				rObj.put("code", "551");
				rObj.put("data", "Trainning 과정 중에 오류가 발생했습니다.\n잠시 후에 다시 한번 시도해주시길 바랍니다.");
			} else if(cmdResult.length() > 0){
				rObj.put("code", "200");
				rObj.put("data", "Trainning을 시작합니다.");
			}
			//if(cmdResult == null) {
			//	cmdResult = "error!";
			//}
		...
		public Object  callCustomInference(String projectId, String taskId, String algorithmId, String gpuIndex, String mode, String modelName, String csvFileName, String parameterJson) {
			String cmd = "docker run --rm -itd --label gpu_id=" + gpuIndex + " --label "
							+ "xlabeller=i_" + projectId + "_" + taskId + " --name xlabeller_i_" + projectId + "_" + taskId + " -v xlabeller_nfs:/xlabeller ca_" + algorithmId + ":latest"
							+ " python -u run_custom_model.py --action inference --mode " + mode + " --params \"" + parameterJson + "\" --gpu_id " + gpuIndex 
							+ " --dataPath /xlabeller/workspace/" + projectId + "/" + taskId + "/imagepath/imagepath.csv"
							+ " --labels /xlabeller/workspace/" + projectId + "/" + taskId + "/class/classes"
							+ " --modelPath /xlabeller/workspace/" + projectId + "/" + taskId + "/model/" + modelName
							+ " --saveResult /xlabeller/workspace/" + projectId + "/" + taskId + "/result/" + csvFileName;
...
//	              String end = "u001B[0m";
		...
		public boolean nfsStatus(String address) {
			...
			nfsStatusCmdOutput = nfsStatusCmdOutput.trim();
			String joinOutput = "map[device::/xlabeller o:addr=" + address + ",rw,nolock type:nfs]";
			logger.info("nfsStatusCmdOutput::" + nfsStatusCmdOutput);
			logger.info("joinOutput::" + joinOutput);
		...
		public boolean nfsJoin(String address) {
			...
			nfsJoinCmdOutput = nfsJoinCmdOutput.trim();
